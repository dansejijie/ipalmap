...
import com.backustech.huitumap.view.BookShelf;
import com.backustech.huitumap.view.IpalmapHelpIndicatorView;
...
public class IpalmapShowMapActivity extends IpalmapActivity {
    ...
    private void getDataAsync() {
        ...
        try{
            ...
            ToastUtils.showToast(mHandler,"请确认是否传入正确参数call_number和book_name");
            bookLoaded=false;
            return;
        }
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ToastUtils.showToast(mHandler,e.getMessage());
                bookLoaded=false;
            }
            @Override
    ...
    private void handleBookJSON(String json){
        ...
        try{
            ...
            e.printStackTrace();
            ToastUtils.showToast(mHandler,"书籍信息处理失败");
            bookLoaded=false;
        }
    ...
    private void report(){
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                hideLoadingDialog();
                if(response.isSuccessful()){//回调的方法执行在子线程。
                    ToastUtils.showToast(mHandler,R.string.iplmap_book_shelf_report);
                }
            }
        });
    }
